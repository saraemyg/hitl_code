...
async def bulk_detect():
    """
    Process all images in the input folder, save annotated images to outputs, and return detection results.
    """
    input_dir = "input"
    output_dir = "outputs"
    os.makedirs(output_dir, exist_ok=True)
    ...
    for image_file in image_files:
        ...
        try:
            ...
            results.append({
                ...
                "output_image": annotated_filename,
                "detections": detections,
                "defect_count": len(detections)
            })
    ...
    metadata_path = os.path.join(output_dir, "bulk_detection_metadata.json")
...
async def upload_images(files: List[UploadFile] = File(...)):
    """
    Save multiple uploaded images to the input folder.
    """
    input_dir = "input"
